#' Penalized Version of Linear Model Prediction
#'
#' @import psych
#' @import glmnet
...
#' @param lambda the value of penalty.
...
linear.regularized.GenCPM <- function(connectome, behavior, x, 
                                      external.connectome = NULL, external.x = NULL,
                                      cv="leave-one-out",
                                      k=dim(connectome)[3], correlation = "pearson",
  ...
  if((cv == "leave-one-out" && k == dim(connectome)[3]) | (cv == "k-fold" && k == dim(connectome)[3])){
    ...
    for(leftout in 1:N){
      ...
      if(edge=="separate"){
        ...
        behav_pred_pos[leftout] <- predict(fit_pos, newx=test_covars_pos, s = opt_lambda_pos)
        behav_pred_neg[leftout] <- predict(fit_neg, newx=test_covars_neg, s = opt_lambda_neg)
        ...
        if(missing(lambda) | length(lambda) < 1){
          ...
          fit_lambda <- cv.glmnet(train_covars, train_behav, alpha=type_alpha,
                                  penalty.factor=c(rep(1, nedge),rep(0, ncol(train_covars)-nedge)))
          opt_lambda <- fit_lambda$lambda.1se

        }else if(length(lambda) > 1){

          fit_lambda <- cv.glmnet(train_covars, train_behav, lambda = lambda,
                                  alpha=type_alpha,
                                  penalty.factor=c(rep(1, nedge),rep(0, ncol(train_covars)-nedge)))
          opt_lambda <- fit_lambda$lambda.1se
        ...
        fit <- glmnet(train_covars, train_behav, lambda = opt_lambda,
                      alpha=type_alpha,
                      penalty.factor=c(rep(1, nedge),rep(0, ncol(train_covars)-nedge)))
        ...
        behav_pred[leftout] <- predict(fit, newx=test_covars, s = opt_lambda)
        behav_actual[leftout] <- test_behav
    ...
    if(cv == "k-fold" | ((cv == "leave-one-out") && (k != dim(connectome)[3]))){
      ...
      if(k-floor(k) != 0){
        ...
        randinds <- sample(rand_index,replace = FALSE)
        ...
        for (fold in 1:k){
          ...
          if(edge=="separate"){
            ...
            behav_pred_pos[fold,] <- predict(fit_pos, newx=test_covars_pos)
            behav_pred_neg[fold,] <- predict(fit_neg, newx=test_covars_neg)
            ...
            behav_pred[fold,] <- predict(fit, newx=test_covars)
            behav_actual[fold,] <- test_behav
        ...
        if(edge=="separate"){
          ...
          behav_pred_pos <- predict(fit_pos, newx=test_covars_pos)
          behav_pred_neg <- predict(fit_neg, newx=test_covars_neg)
          ...
          behav_pred <- predict(fit, newx=test_covars)
