  ...
  if(missing(external.connectome)){
    ...
    if((cv == "leave-one-out" && k == dim(connectome)[3]) | (cv == "k-fold" && k == dim(connectome)[3])){
      ...
      for (leftout in 1:N){
        ...
        cpm <- train.GenCPM(train_array, train_behav, train_x, fit = "linear",
      ...
      if(k-floor(k) != 0){
        ...
        for (fold in 1:k){
          ...
          if(edge == "separate"){
            ...
            if((NA %in% fit_pos) == FALSE){
              behav_pred_pos[fold,] <- predict(fit_pos, newdata=test_pos)
            }
            else{
    ...
    if(edge == "separate"){
      ...
      if((NA %in% fit_pos) == FALSE){
        behav_pred_pos <- as.vector(predict(fit_pos, newdata=test_pos))
      }
      else{
        behav_pred_pos <- NA
      }
      ...
      if((NA %in% fit_neg) == FALSE){
        behav_pred_neg <- as.vector(predict(fit_neg, newdata=test_neg))
      }
      else{
        behav_pred_neg <- NA
      }
      ...
      if((NA %in% fit_combined) == FALSE){
        behav_pred <- as.vector(predict(fit_combined, newdata=test_combined))
      }
      else{
        behav_pred <- NA
      }
