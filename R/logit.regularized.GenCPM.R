#' Penalized Version of Logistic Model Prediction
#'
#' @import psych
#' @import glmnet
...
logit.regularized.GenCPM <- function(connectome, behavior, x, 
                                     external.connectome = NULL, external.x = NULL,
                                     cv="leave-one-out", k=dim(connectome)[3], 
                                     correlation = "pearson", thresh=.01,
                                     edge="separate", type="lasso", lambda=NULL,
                                     alpha=NULL, seed=1220){
      ...
      if(edge=="separate"){
        ...
        if(missing(lambda) | length(lambda) < 1){
          ...
          fit_lambda_pos <- cv.glmnet(train_covars_pos, train_behav,
                                      family="binomial", alpha=type_alpha,
                                      type.measure="auc",
          ...
          fit_lambda_pos <- cv.glmnet(train_covars_pos, train_behav,
                                      ...
                                      alpha=type_alpha, type.measure="auc",
                                      penalty.factor=c(rep(1, nedge_pos),rep(0, ncol(train_covars_pos)-nedge_pos)))
          opt_lambda_pos <- fit_lambda_pos$lambda.min
        ...
        fit_pos <- glmnet(train_covars_pos, train_behav, family="binomial",
                          lambda = opt_lambda_pos, alpha=type_alpha,
                          penalty.factor=c(rep(1, nedge_pos),rep(0, ncol(train_covars_pos)-nedge_pos)))
        fit_neg <- glmnet(train_covars_neg, train_behav, family="binomial",
                          lambda = opt_lambda_neg, alpha=type_alpha,
                          penalty.factor=c(rep(1, nedge_neg),rep(0, ncol(train_covars_neg)-nedge_neg)))

        # edge selection by glmnet

        coef_pos <- data.matrix(coef(fit_pos))[c(2:nedge_pos+1),]
        coef_neg <- data.matrix(coef(fit_neg))[c(2:nedge_neg+1),]

        selected_edges_pos[[leftout]] <- edge_index_pos[which(coef_pos > 0)]
        selected_edges_neg[[leftout]] <- edge_index_neg[which(coef_neg < 0)]

        # predict TEST subs with the best lambda param

        behav_pred_pos[leftout] <- predict(fit_pos, newx=test_covars_pos, s = opt_lambda_pos, type="response")
        behav_pred_neg[leftout] <- predict(fit_neg, newx=test_covars_neg, s = opt_lambda_neg, type="response")
        ...
        if(missing(lambda) | length(lambda) < 1){
          ...
          fit_lambda <- cv.glmnet(train_covars, train_behav, family="binomial",
                                  alpha=type_alpha, type.measure="auc",
        ...
        fit <- glmnet(train_covars, train_behav, family="binomial",
                      lambda = opt_lambda, alpha=type_alpha,
                      penalty.factor=c(rep(1, nedge),rep(0, ncol(train_covars)-nedge)))

        # edge selection by glmnet

        coef <- data.matrix(coef(fit))[c(2:(nedge+1)),]

        selected_edges_pos[[leftout]] <- edge_index[which(coef > 0)]
        selected_edges_neg[[leftout]] <- edge_index[which(coef < 0)]

        # predict TEST subs with the best lambda param

        behav_pred[leftout] <- predict(fit, newx=test_covars, s = opt_lambda, type="response")
        behav_actual[leftout] <- test_behav
  ...
  if(cv == "k-fold" | ((cv == "leave-one-out") && (k != dim(connectome)[3]))){
    ...
    if(k-floor(k) != 0){
      ...
      for (fold in 1:k){
        ...
        if(edge=="separate"){
          ...
          selected_edges_pos[[fold]] <- edge_index_pos[which(coef_pos > 0)]
          selected_edges_neg[[fold]] <- edge_index_neg[which(coef_neg < 0)]

          # predict TEST subS with the best lambda param

          behav_pred_pos[fold,] <- predict(fit_pos, newx=test_covars_pos, s = opt_lambda_pos, type="response")
          behav_pred_neg[fold,] <- predict(fit_neg, newx=test_covars_neg, s = opt_lambda_neg, type="response")
          ...
          selected_edges_pos[[fold]] <- edge_index[which(coef > 0)]
          selected_edges_neg[[fold]] <- edge_index[which(coef < 0)]

          # predict TEST subs with the best lambda param

          behav_pred[fold,] <- predict(fit, newx=test_covars, s = opt_lambda, type="response")
          behav_actual[fold,] <- test_behav
    ...
    if(edge=="separate"){
      ...
      selected_edges_pos <- edge_index_pos[which(coef_pos > 0)]
      selected_edges_neg <- edge_index_neg[which(coef_neg < 0)]
      
      # predict TEST subs with the best lambda param
      
      behav_pred_pos <- predict(fit_pos, newx=test_covars_pos, type="response")
      behav_pred_neg <- predict(fit_neg, newx=test_covars_neg, type="response")
      ...
      fit <- glmnet(train_covars, train_behav, 
                    family = "binomial", lambda = opt_lambda,
                    alpha=type_alpha,
                    penalty.factor=c(rep(1, nedge),rep(0, ncol(train_covars)-nedge)))
      ...
      selected_edges_pos <- edge_index[which(coef > 0)]
      selected_edges_neg <- edge_index[which(coef < 0)]
      
      # predict TEST subs with the best lambda param
      
      behav_pred <- predict(fit, newx=test_covars, type="response")
